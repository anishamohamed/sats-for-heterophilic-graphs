...
class HeterophilousGraphWrapper(pl.LightningModule):
    ...
    def training_step(self, data, data_idx):
        ...
        y = data.y.squeeze()# [data.train_mask[:, self.mask]].squeeze()

        loss = self.criterion(output, y)
        self.log("train/loss", loss.item(), prog_bar=True)
        print(f"\nTrain Predictions: {torch.argmax(output, dim=-1)[:100]}, labels = {torch.unique(torch.argmax(output, dim=-1)[:100])}")
        correct = torch.sum(torch.argmax(output, dim=-1) == y)
        print(f"output: {output.shape}, y: {y.shape}, correct: {correct} / {len(y)}")
        self.log("train/acc", correct / len(y))

        return loss

    def validation_step(self, data, data_idx):
        output = self(data) # self(data)[data.val_mask[:, self.mask]]
        y = data.y.squeeze() # data.y[data.val_mask[:, self.mask]].squeeze()

        loss = self.criterion(output, y)
        self.log("val/loss", loss.item(), prog_bar=True)
        predictions = torch.argmax(output, dim=-1)
        print(f"\nVal Predictions: {predictions[:100]}, labels = {torch.unique(predictions)}")
        correct = torch.sum(torch.argmax(output, dim=-1) == y)
        print(f"output: {output.shape}, y: {y.shape}, correct: {correct} / {len(y)}")
    ...
    def test_step(self, data, data_idx):
        ...
        y = data.y.squeeze() # [data.test_mask[:, self.mask]].squeeze()

        loss = self.criterion(output, y)
        self.log("test/loss", loss.item(), prog_bar=True)
        print(f"\nTest Predictions: {torch.argmax(output, dim=-1)[:100]}")
        correct = torch.sum(torch.argmax(output, dim=-1) == y)
